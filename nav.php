<nav class="navbar navbar-expand-lg navbar-dark bg-dark p-3 shadow-lg">
	<div class = "container">
	<a class="navbar-brand" href="index.php">Lucas Gabriel</a>
	<button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation"><span class="navbar-toggler-icon"></span></button>

		<div class="collapse navbar-collapse" id="navbarSupportedContent">
			<ul class="navbar-nav mr-auto">
				<li class="nav-item active"><a class="nav-link" href="index.php">Início <span class="sr-only">(current)</span></a></li>
				<li class="nav-item"><a class="nav-link" href="feed.php">Galeria 01</a></li>
				<li class="nav-item"><a class="nav-link" href="feed_02.php">Galeria 02</a></li>
				<!--li class="nav-item dropdown">
					<a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Mais</a>
					<div class="dropdown-menu" aria-labelledby="navbarDropdown">
					<a class="dropdown-item" href="#">Usuários</a>
					<a class="dropdown-item" href="#">Catálogo</a>
					<div class="dropdown-divider"></div>
					<a class="dropdown-item" href="#">Github</a>
					</div>
				</li-->
			</ul>
			<form class="form-inline my-2 my-lg-0">
				<a target = "blank" href = "https://github.com/lucgbrl/CompiladoProjetoFinalTCC" class="btn btn-outline-light my-2 my-sm-0"><i class = "fa fa-user-circle"></i> Github</a>
			</form>
		</div>
	</div>
</nav>
